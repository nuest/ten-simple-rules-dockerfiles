...
Such practices are not part of generic containerisation tutorials and are not present in published `Dockerfile`s, which are often used as templates by novices.
...
These layers can be used for caching across image builds.
The images can then be run as stateful containers, which can be stopped, restarted and purged.
...
Importantly, the researcher should clearly state the platforms (by way of one or more Docker containers) that a workflow has been tested on.
...
While some organizations can be trustworthy in terms of content, and reliably update containers with security fixes (e.g., `rocker/r-ver`), for most individual accounts that build containers, it's likely that containers will not be updated regularly.
...
Doing this kind of trace is essential because it makes all of the steps that were taken to generate your container and who took them.
...
The documentation and tools of container registries, e.g., [Docker Hub](https://docs.docker.com/docker-hub/builds/) or [GitLab](https://docs.gitlab.com/ee/user/packages/container_registry/index.html#build-and-push-images) instances, but also of CI platforms, e.g. [GitHub actions](https://github.com/actions/starter-workflows/tree/master/ci), a GitHub actions for `repo2docker` [@husain_repo2docker-action_2019; @scottyhq_repo2docker-githubci_2019], or [CircleCI](https://circleci.com/orbs/registry/orb/circleci/docker#commands-build), can help you get started.
...
Use `\` to break a single command into multiple lines to ensure that no single line gets too long.
...
Labels are useful to provide a more structured form of documentation about software, especially for users of supporting infrastructure that might not expose the actual `Dockerfile` - see Rule&nbsp;6 for details.
...
A `RUN` statement longer than a page requires scrolling.
...
# 5. Bind mount data and control code {-}
...
If the target path exists in the image, the bind mount will replace it for the specific container.
...
There are several projects (https://codemeta.github.io/, https://citation-file-format.github.io/) focused on metadata citation, and you should consider using one if it seems appropriate for your use case.
...
DN conceived the idea, wrote the first darft, and contributed to all rules.
VS wrote the first draft based on notes and contributed to all rules.
...
This articles was written collaboratively on GitHub, where all contributions in form of text or discussions comments are documented: [https://github.com/nuest/ten-simple-rules-dockerfiles/](https://github.com/nuest/ten-simple-rules-dockerfiles/).
