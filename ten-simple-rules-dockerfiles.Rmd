...
However, a Research Software Engineer's (RSE) skill level is not the target audience for this work, but we hope to provide a useful resource for more advanced container users as well.
We encourage you to reach out to your local or national RSE community if your questions on sofware development in research go beyond the rules of this work.
...
`Dockerfile`s are machine- *and* human-readable recipes for building images, comparable to `Makefile`s [@wikipedia_contributors_make_2019].
...
Therefore, for security, transparency, and reproducibility, you should only use images where you have access to the `Dockerfile`, e.g., by using automated builds on Docker Hub.
...
Avoid to `docker push` a locally built image, because that counteracts the considerations outlined above.
...
Generally, aim to design the `RUN` instructions so that each performs one scoped well-documented action, e.g., download, compile, and install _one tool_.
...
These instructions are not limited to `docker <command>`, but include usage of bespoke scripts, `Makefile`s, or `docker-compose` (see \ruleref{rule:interactive} and \ruleref{rule:usage}).
