...
Never include such data in an image.
...
If a VCS repository with a `Dockerfile` is linked to an automated build, then a CI pipelien can build images and execute a validating run the new image, e.g., using contained demo data (see&nbsp;\ruleref{rule:mount}).
