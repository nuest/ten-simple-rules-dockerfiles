...
\ztitlerefsetup{title=2}\zlabel{rule:base}
...
This is a very good practice and helpful, though you should consider the externalization of content to outside of the `Dockerfile` (see \ruleref{rule:mout}).
...
How you bind external resources into the container should be included in the example commands (see \ruleref{rule:mount}).
...
If part of the software developed for a specific analysis is published as a conventional package you should follow \ruleref{rule:pinning} for installing it from a public repository, but you may also want to install it from source after copying it into the image.
...
The exported image and a version controlled `Dockerfile` together allow you to freely experiment and continue development of your workflow and keeping the image up to date, e.g., updating versions of pinned dependencies (see \ruleref{rule:pin}) and regular image building (see above).
