...
- researchers need skills to handle virtual laboratories to ensure understandability and reproducibility
...
- the environment can then _host_ executable and human-readable notebooks with the scientific workflow [@rule_ten_2019]
- several Ten Simple Rules articles answer these challenges [@sandve_ten_2013; @taschuk_ten_2017, @rule_ten_2019]
- there are tutorials for using containers for reproducible research:
  ...
  - ...
- there are platforms using containers for transparency and reproducibility
  - BinderHub
  - o2r
  - Whole Tale
  - ...
...
**BUT** many scientists are not developers, and not all good practices found in tutorials for Docker apply to reproducible and transparent research
...
- _never_ use `:latest`
- pick a base image with a Linux distribution that supports the required software stack, and ideally that is widely used in your community, e.g. Ubuntu for geospatial research
...
# 5. Use labels and build arguments for relevant links and metadata {-}
...
# 6. Enable interactive usage and one-click execution {-}
...
# 7. End the `Dockerfile` with build and run commands
