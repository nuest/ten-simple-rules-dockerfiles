...
Even if they are not sure how the technology behind them actually works, they can leverage containerisation, but should craft their own definition of computing environments following good practices.
...
Mounting these files is preferable to using the `ADD`/`COPY` instructions in the `Dockerfile`, because files persist when the container instance or image is removed from your system, and the files are more easily accessible to other users if published in an online research compendium outside of the image.
...
If the container is linked to an automated build, then pushing updates to a VCS repository with a `Dockerfile` can easily trigger an build image build and a validating execution of the new images within a CI pipeline.
