...
For this reason, it is necessary to pin the dependency versions. In the case of having few packages, it may be simplest to write the install steps and versions directly into the `Dockerfile` (also for clarity, see \ruleref{rule:clarity}):
...
This modularization may reduce readability, but provides more flexibility in facillitating different ways of building a reproducible environment, provided the dependency file is under version control in the same repository (see \ruleref{rule:publish}).
You can also use package managers to install software from source code `COPY`ied into the image (see&nbsp;\ruleref{rule:mount}).
Finally, you can use many package managers to install software from source obtained from external code management repositories, e.g., installing a tool from a specific GitHub branch or version tag.
Be aware of the risk that such installations may later fail, especially when the repositories are out of your control. However, these concerns can be mitigated by running the installation command with the full URL (including the specific version tag or commit hash) which is helpful in troubleshooting if problems arise.
...
We recommend ensuring that the image works without specifying any users, and to document precisely if your image deviates from that.
...
It is better to insert data files from the local machine into the container at run time, and use the image primarily for the software and dependencies.
...
Ignoring data files also speeds up the build in case of very large files or many small files.
You should include dummy or test data in the image to be able to ensure that a container is functional without a larger custom dataset, e.g., for automated tests or instructions in the user manual (see also "functional testing logic" in @gruening_recommendations_2019).
For all these cases you should provide clear instructions for users in the `README` file on how to obtain the actual or dummy data.
When publishing your workspace, e.g., on Zenodo, having datasets outside of the container makes them more accessible to others, for example for reuse or analysis.
...
```bash
# mount directory
docker run --mount type=bind,source=/home/user/project,target=/project mycontainer

# mount directory as read-only
docker run --mount type=bind,src=/home/user/project,dst=/project,readonly mycontainer

# mount multple directories, one with write access relative to current path (Linux)
docker run --mount type=bind,src=/homse/user/article-x-supplement/data,dst=/data,readonly \
...
To support advanced custom configuration, it's helpful to expose settings via a configuration file, which can be bind mounted from the host [@verstegen_pluc_mozambique_2019], via environment variables [@knoth_reproducibility_2017], or via wrappers using Docker, such as Kliko [@molenaar_klikoscientific_2018].
...
Part of your project template should be a protocol for publishing the `Dockerfile` and even an export of the image in a suitable location, e.g., a container registry or data repository, taking into consideration how your workflow can receive a DOI for citation.
