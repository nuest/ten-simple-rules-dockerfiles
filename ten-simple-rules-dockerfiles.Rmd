...
A `Dockerfile` consists of a sequence of instructions to copy files and install software, which add layers to the image.
...
Well-maintained includes the tool's own stability and usability, but also that suitable base images are used, likely from the official Docker library [@docker_inc_official_2019], to ensure that the container has the most recent security fixes for the operating system in question.
Third, you may want to write you own `Dockerfile` from scratch if these tools do not suffice for your needs.
