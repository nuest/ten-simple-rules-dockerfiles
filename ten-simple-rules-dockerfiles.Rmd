...
In this field, because a researcher's (CRB comment: instead of "researcher's", maybe "study's" or "paper's" or something similar?) actual contribution to knowledge includes the full computing environment that produced a result [@donoho_invitation_2010], containerisation is needed.
...
We encourage you to reach out to your local or national RSE (CRB comment: Should this abbreviation be "RSEng"?) community if you have questions on software development in research that go beyond the rules of this work.
...
Such tools have likely thought about and implemented good practices, and they may even have incporporated newer practices when reapplied at a later point in time.
...
For example [this `Dockerfile` by Kaggle](https://github.com/Kaggle/docker-rstats/blob/master/Dockerfile) does a good job of explaining the reasoning and steps of the contained instructions.
...
You can view all labels with the [`docker inspect`](https://docs.docker.com/engine/reference/commandline/inspect/) command.
Listing&nbsp;3 shows the most relevant labels for data science workflows.
...
Thus, usage instructions (CRB comment: I wasn't sure what "It helps others" meant in the original text, so I tried to revise. OK changes here? Check and revise further as needed) help others, because they quickly get them running your workflow and interacting with the container in the intended way without reading all of the instructions (a ["tl;dr"](https://en.wikipedia.org/wiki/Wikipedia:Too_long;_didn%27t_read)-kind of usage).
...
_Installing from source_ is a useful way to install very specific versions, but it comes at the cost of needing to build libraries.
...
Because all of the above files require the user to inspect the file or the build to see what is installed, we suggest that when you only have a few packages, write the install steps and versions directly into the `Dockerfile` (also for clarity, see \ruleref{rule:clarity}).
...
This modularisation may pose a risk for understandability and consistency, but it can be mitigated by carefully managing all these files in the same version-controlled project.
You can also use package managers to install software from source code `COPY`ied into the image (see&nbsp;\ruleref{rule:mount}).
And, finally, you can use many package managers to install software obtained from code management repositories, e.g., installing a specific tool identified by a GitHub version tag or commit hash.
Be aware of the risk of depending on such repositories, especially if they are out of your control, but the installation command with a full URL should allow readers of your `Dockerfile` to dig deeper if problems arise.
Such an installation from a source gives you much freedom, including the freedom to shoot yourself in the foot!
...
As a final note on software installation, you should be aware of the [`USER` instruction](https://docs.docker.com/engine/reference/builder/#user) in a `Dockerfile` and how your base image might adjust the user to enable particular usages. (CRB comment: I don't really know what "adjust the user" means here. Can you clarify?)
It is common to use images with the default user `root`, which is required for installing system dependencies.
We recommend making sure that the image works without specifying any users, and, if your image deviates from that, we suggest you document it precisely.
...
It is better to include data files and custom scripts from the local machine in the container at runtime, and use the container image primarily for the software and dependencies.
...
If you want to add local files to the container, we prefer `COPY` because it is explicit and you do not need `ADD`'s extra features, which are (a) URL as a source, which can be used instead of a `RUN wget` or `RUN curl` to download files and (b) a local [tar file](https://en.wikipedia.org/wiki/Tar_(computing)) as a source, which you should avoid because such an archive file will be harder for future readers of your `Dockerfile` to make sense of and cannot be properly version controlled.
...
If the code and `Dockerfile` are in the same repository, the origin and results of the `COPY` instruction and subsequent installation code will be understandable in the future.

Storing _data files_ outside of the container allows for handling of very large datasets and datasets with data worthy of protection, e.g., proprietary data or private information.
...
Ignoring data files also speeds up the build in cases where there are very large files or many small files; for many small files, one can thus ignore, for example, the `.git` folder of the Git version control system.
When ignoring data (CRB comment: Right idea here? Revise further as needed), you should include dummy or test data into the image to ensure that a container is functional without a larger custom dataset, e.g., for automated tests or instructions in the user manual (see also "functional testing logic" in @gruening_recommendations_2019).
For all these cases, you should provide clear instructions for users in the `README` on how to obtain actual or dummy data.
...
A possible weakness with using containers is that they can only provide one default command and entrypoint.
...
If your workflow can be executed via a command line client, you may use that to validate correct functionality of an image in automated builds, e.g., by using a small toy example and checking the output, by checking successful responses from HTTP endpoints provided by the container, such as with an HTTP response code of `200`, or by using a controller such as Selenium [@selenium_2019]. (CRB comment: I revised the text after "e.g." to indicate that there are three examples here - the toy example, the checking successful responses from HTTP endopoints example, and the controller example - check these revisions and revise further as needed.)
...
This variety of approaches renders seemingly more convenient uncontainerised environments, i.e., just using one's local machine is unnecessary in favour of reproducibility and portability.(CRB comment: OK revision to the phrase after "i.e."? Revise further as needed.)

# 9. Use one Dockerfile per project and publish it with a version control system {-}
...
We encourage researchers to follow these steps taken by their peers to use `Dockerfile`s to practice reproducible research, and we encourage them to promote change in the way scholars communicate towards an open and friendly "preproducibility" [@stark_before_2018].
