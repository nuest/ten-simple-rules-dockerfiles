...
Containers can grow quickly in size, particularly when data is involved, and you should keep this in mind. Smaller images are often indicated by having `slim` or `minimal` as part of the tag.  If you need to build smaller containers, consider a `busybox` base or a multi-stage build, which allows you to selectively copy files from one build step to another. Also take into account the libraries that you actually need.
