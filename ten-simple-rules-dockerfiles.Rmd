...
RUN wget http://download-server.com/usefule_software/$USEFULSOFT_VERSION/$USEFULSOFT_VERSION.zip && \
  ...
  bash install.sh --tempdir $INSTALL_TEMP --target $INSTALL_TARGET && \
  cd .. && \
  rm -r $INSTALL_TEMP $USEFULSOFT_VERSION $USEFULSOFT_VERSION.zip

# Puth UsefulSoft tool on the path for subsequent instructions
ENV PATH $PATH:$INSTALL_TARGET
