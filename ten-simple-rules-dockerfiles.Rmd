...
author:
  ...
  - name: Benjamin Evans
    ...
    affiliation: "School of Psychological Science, University of Bristol, GB"
...
author_summary: |
  ...
  A helpful abstraction for capturing the computing environment is something called a _container_, whereby a container is created from a set of instructions in a recipe.
...
In this field, because a researcher's (CB comment: instead of researcher's, maybe study's or paper's or something similar?) actual contribution to knowledge includes the full computing environment that produced a result [@donoho_invitation_2010], containerisation is needed.
...
Based on the article by Guening et&nbsp;al. [@gruening_recommendations_2019], which gives very helpful recommendations for packaging reusable software in a container, here we introduce these instructions for the popular `Dockerfile` format, and we focus on containers that encapsulate data science workflows. (CB comment: I wasn't sure how to interpret the link between the Guening article and the intstructions you give here, so I tried to revise to make this link clearer. If the revisions are not exactly what you mean, revise further here.)
...
Since containers that you eventually share with others can only run open source software, tools like Mathematica and MATLAB are outside the scope of this example; other workflows outside the scope of reproducible research are those that do not support scripted execution, as they do not fit well with containerisation.
Furthermore, also outside the scope of these rules are workflows that interact with many petabytes of data and are executed in high performance computing (HPC) infrastructures.
...
For this article, we focus on workflows that typically run on single machine, e.g., a researcher's laptop computer or a virtual server.
Readers might scope the data requirement to less than a terabyte and the compute requirement to a machine with 16 cores running over the weekend.
...
A `docker-compose.yml` configuration file allows for defining volumes, environment variables, and exposed ports and helps users stick to  _one purpose per container_, which often means one process running in the container, and to combine existing stable building blocks instead of bespoke massive containers for specific purposes.
...
We encourage you to reach out to your local or national RSE (CB comment: Should this abbreviation be RSEng?) community if you have questions on software development in research that go beyond the rules of this work.
...
Images have **tags** associated with them with specific meanings, e.g., a version indicator such as `3.7` or `dev`, or variants such as `slim` attempting to reduce image size.
...
The following list a selection of communities that produce widely used regularly updated images, including ready-to-use images with preinstalled collections of software configured to work out of the box.
...
# 3. Format intentionally and favour clarity
...
While you will find `Dockerfile`s using [_build-time variables_](https://docs.docker.com/engine/reference/commandline/build/#set-build-time-variables---build-arg) to dynamically change parameters at build time, such a customisation option reduces clarity for data science workflows.
...
However, a script or `Makefile` might be required to not forget setting the argument (see \ruleref{rule:usage}).
...
_Listing&nbsp;3_: Commonly used labels.
...
These variables persist inside the container and can be useful for example for (i) setting software versions or paths and reusing them across multiple instructions to avoid mistakes, (ii) specifying metadata intended to be discovered by installed libraries or software, or (iii) adding binaries to the path (`PATH`) or library path (`LD_LIBRARY_PATH`).
You should be careful to distinguish these environment variables from those that might vary and be required at runtime.
For runtime environment variables, either to set a new variable or override one set in the container, you can use the `--env` parameter of `docker run`.
...
RUN wget http://download-server.com/usefule_software/$USEFULSOFT_VERSION/$USEFULSOFT_VERSION.zip && \
  unzip $USEFULSOFT_VERSION.zip && \
  cd $USEFULSOFT_VERSION && \
  bash install.sh --target $USEFULSOFT_INSTALLDIR && \
  cd .. && \
  rm -r $USEFULSOFT_VERSION $USEFULSOFT_VERSION.zip
...
_Listing&nbsp;4_: Environment variables.
...
You can use comments to visually separate these sections in your file (cf. Listing&nbsp;1).
...
One reason you will encounter base images running a non-root user (popular examples are the Jupyter and Rocker images) is to avoid permission problems when mounting files into the container, especially for "output" files (see&nbsp;\ruleref{rule:mount}).
...
This variety of approaches render seemingly more convenient uncontainerised environments, i.e. just using the local machine, unnecessary in favour of reproducibility and portability.
...
While there are exceptions to the rule (cf.&nbsp;@kim_bio-docklets_2017), it is generally feasible to provide one `Dockerfile` per project.
Alternatively, you could write multiple `Dockerfile`s starting `FROM` one another, i.e., write your own base images.
However, because multiple files scatter information across multiple places, we recommend avoiding this at the cost of a longer `Dockerfile`, which can be mitigated with formatting (see&nbsp;\ruleref{rule:formatting}).

It is likely going to be the case that over time you will work on projects and develop images that are similar in nature to one another.
...
Instead, consider contributing to existing suites of images (see \ruleref{rule:base}) and improving these for your needs instead of building your own independent solution.
...
DN conceived the idea and contributed to conceptualisation, methodology, and writing - original draft.
VS contributed to conceptualisation, methodology, and writing - original draft.
