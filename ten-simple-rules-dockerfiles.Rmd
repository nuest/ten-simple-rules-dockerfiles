...
header-includes:
  ...
  - \providecommand{\myedit}[2]{{\color{blue}#2 [#1]}}
csl: plos.csl
Submission notes:
...
A `Dockerfile` is a machine- _and_ human-readable recipe ~~for building images~~, comparable to a `Makefile` [@wikipedia_contributors_make_2019]\myedit{1.1}{, for building \emph{images}}.
Here, images \myedit{1.1}{are executable files that} include the application, e.g., the programming language interpreter needed to run a workflow, and the system libraries required by an application to run.
...
\myedit{1.1}{Once an image is built or downloaded, it is then launched as a running instance known as a \emph{container}.
...
A visual analogy for building and running a container is provided in Figure&nbsp;\ref{fig:analogy}.
...
knitr::include_graphics("figures/analogy.png")
```
...
You can look for information about images in (a) the documentation of the software you intend to use, (b) the Docker image registry _Docker Hub_, [https://hub.docker.com/](https://hub.docker.com/), or (c) the source code projects of the  software being used, as many developers today rely on containers for development, testing, and teaching.
...
Because `repo2docker` is deterministic, the environments are the same (see&nbsp;~~Rule 6~~\myedit{2.2}{\ruleref{rule:pinning}} for ensuring the same software versions).
...
For any tool that you use, be sure to look at documentation for usage and configuration options, and look for options to add metadata (e.g., labels see&nbsp;~~Rule 3~~\myedit{2.2}{\ruleref{rule:document}}).
...
\myedit{1.2}{Content spread across more and shorter lines also improves readability of changes in version control systems.}
...
Second, clarity ~~is nearly always more important than brevity~~\myedit{3.1}{of the steps within a Dockerfile is most important, and if it requires verbosity or adds to the final image size, that is an acceptable tradeoff}.
For example, if your container uses a script to run a complex install routine, instead of removing it from the container upon completion ~~,which is~~ \myedit{3.1}{(a practice} commonly seen in production `Dockerfile`s aiming at small image sizes, cf.&nbsp;@gruening_recommendations_2019 \myedit{3.1}{)}, you should keep the script in the container for a future user to inspect; the script size is negligible compared to the image size.
One common pattern you will encounter is a single and very lengthy `RUN` instruction chaining multiple commands~~,which installs~~ \myedit{3.1}{to install} software and clean~~s~~ up afterwards.
...
All instructions can be grouped starting with a short comment, which also makes it easier to spot changes if your `Dockerfile` is managed in some version control system (see ~~Rule 9~~\myedit{2.2}{\ruleref{rule:publish}}).
...
\begin{lstlisting}[language=docker,caption={Partial \texttt{Dockerfile} with examples for helpful comments. \myedit{1.3}{}},breaklines=true,label={lst:comments}]
...
...
## \myedit{2.1}{Version Control}

\myedit{2.1}{Software can often be installed directly from a version controlled repository (e.g., GitHub, GitLab, or Mercurial). 
...
\myedit{2.1}{In the case that you want to clone and checkout a specific commit, you can use the \texttt{checkout} command.}
...
This modularisation may reduce readability, but provides more flexibility in facilitating different ways of building a reproducible environment, provided the dependency file is under version control in the same repository (see ~~Rule 9~~\myedit{2.2}{\ruleref{rule:publish}}).
...
How your container expects external resources to be mounted into it should be included in the example commands (see ~~Rule 3~~\myedit{2.2}{\ruleref{rule:document}}).
...
If you use a `Makefile`, then keep it in the same repository (see \ruleref{rule:mount}) and include instructions for its usage (see ~~Rule 3~~\myedit{2.2}{\ruleref{rule:document}}).
To support ~~advanced custom configuration~~\myedit{}{more complex configuration options}, it is helpful to expose settings via a configuration file, which can be bind mounted from the host [@verstegen_pluc_mozambique_2019], via environment variables (see ~~Rule 6~~\myedit{2.2}{\ruleref{rule:document}} and @knoth_reproducibility_2017), or via wrappers using Docker, such as Kliko [@molenaar_klikoscientific_2018].
...
While there is no firm rule, if you use a container daily, is good practice to rebuild that container every one or two weeks; this helps identify breaking changes early and prevents mutliple issues compounding on each other.
...
In you need a setup or configuration for the first two habits, it is good practice to provide a `Makefile` alongside your `Dockerfile`, which can capture the specific commands.
...
Sharing the actual image via a registry and a version-controlled `Dockerfile` together allows you to freely experiment and continue developing your workflow and keep the image up to date, e.g., updating versions of pinned dependencies (see ~~Rule 6~~\myedit{2.2}{\ruleref{rule:pinning}}) and regular image building (see above).
...
To demonstrate the ten rules, we maintain a collection of annotated example `Dockerfile`s in the `examples` directory of this article's GitHub repository.
