...
research} besides generic best practices {[}12,13{]}. This article
...
\texttt{docker-compose} {[}14{]} in the case of needing container
...
science {[}16{]}. Research Software Engineers (RSEs) are not the target
...
Makefiles {[}25{]}, are meant to be useful for both humans to read and
...
used, likely from the official Docker library {[}29{]}, to ensure that
...
small image size, you should keep the script in the container for a
...
versions, license, and maintainer contact information are commonly seen
and very useful if a \texttt{Dockerfile} is discovered out of context.
While you can add arbitrarily complex information with labels, for data
science scenarios the user-facing documentation is much more important.
Relevant metadata that might be more utilized with future tools includes
global identifiers such as \href{https://orcid.org/}{ORCID identifiers},
DOIs of the research compendium
...
installing it. Consider using the suitable package system with pinned
versions even for small scripts if the functions are reusable across
datasets or workflows by you or others. You should avoid installing
software packages from source after \texttt{COPY}ing the code into the
image, because the connection between the file outside of the image and
the one copied in is easily lost
...
The Scientific Filesystem {[}46{]}, have been developed to expose
...
potentially capture build and run commans a \texttt{Makefile} {[}25{]}.
...
host {[}45{]}, via environment variables {[}47{]}, or via wrappers using
Docker, such as Kliko {[}48{]}.
...
controller such as Selenium {[}50{]}.
...
templates {[}57{]}, project starter kits {[}REF{]}, or community
templates (e.g., {[}58{]}) can provide required files, e.g., for
...
``preproducibility'' {[}64{]}. So please, make a best effort with your
