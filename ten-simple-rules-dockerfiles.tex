...
# > docker run -it -p 80:8787 -e PASSWORD=ten --volume $(pwd)/input:/input great_workflow:1.0.0
# Run the workflow:
# > docker run -it --name gwf great_workflow:1.0.0 /work/myscript.sh
# Extract the data:
...
\addcontentsline{toc}{section}{8. Make it one-click runnable}
...
\hyperref[{rule:pinning}]{Rule~\ztitleref{rule:pinning}} and {[}52{]}),
...
separate these sections in your file (cf.~Listing~\ref{lst:full}):
...
``preproducibility'' {[}63{]}, which values openness, transparency and
...
Clark who provided feedback on the preprint {[}64{]} of this paper.
...
64. Nüst D, Sochat V, Marwick B, Eglen S, Head T, Hirst T. Ten Simple
