...
regularly. It is even possible that an image or a \texttt{Dockerfile}
may disappear, or that an image is published with malicious intent,
though we have not heard of any such case in academia. Therefore, for
security, transparency, and reproducibility, you should only use images
where you have access to the \texttt{Dockerfile}. We suggest to save a
copy of the \texttt{Dockerfile} within your project (see
...
software environments, e.g., machine learning tool stacks, or a specific
...
software, or (ii) adding binaries to the path (\texttt{PATH}) or library
...
at runtime. For runtime environment variables, either to set a new
variable or override one set in the container, you can use the
...
\KeywordTok{ENV}\NormalTok{ USEFULSOFT_VERSION 1.0.0}
\KeywordTok{ENV}\NormalTok{ INSTALL_TEMP=/tmp/useful \textbackslash{}}
\NormalTok{  INSTALL_TARGET=/usr/bin}
...
\NormalTok{  bash install.sh --tempdir $INSTALL_TEMP --target $INSTALL_TARGET && \textbackslash{}}
\NormalTok{  cd .. && \textbackslash{}}
\NormalTok{  rm -r $INSTALL_TEMP $USEFULSOFT_VERSION $USEFULSOFT_VERSION.zip}

\CommentTok{# Puth UsefulSoft tool on the path for subsequent instructions}
\KeywordTok{ENV}\NormalTok{ PATH $PATH:$INSTALL_TARGET}
...
increase the impact and sustainability of your work (cf.~{[}56{]}).
...
While there are exceptions to the rule (cf.~{[}57{]}), it is generally a
simple and clear approach to provide one \texttt{Dockerfile} per
project. Alternatively, you could build, or even better contribute to
existing, \emph{image stacks} to serve different use cases with a common
base image, but this reduces understandability and scatters information
across multiple places. We recommend avoiding this at the cost of a
longer \texttt{Dockerfile}, which can be mitigated with consistent form
...
slate for a new project. As an example, cookie cutter templates {[}58{]}
or community templates (e.g., {[}59{]}) can provide required structure
and files, e.g., for documentation, CI, and licenses, for getting
started. If you decide to build your own cookie cutter template,
consider collaborating with your community during development of the
standard to ensure it will be useful to others. Part of your project
template should be a protocol for publishing the \texttt{Dockerfile} and
even exporting the image to a suitable location, e.g., a container
registry or data repository, taking into consideration how your workflow
can receive a DOI for citation.
...
1632/17-1. DN and SJE are supported by a Mozilla mini science grant. We
thank Dav Clark who provided feedback on the preprint {[}66{]} of this
paper.
