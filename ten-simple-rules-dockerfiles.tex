...
\label{lst:comments} \emph{Listing~2}: Example comments.
...
\CommentTok{# This ARG instruction adds a build-time variable with the default value 'unspecified'}
\CommentTok{# Set it during `docker build` as follows to insert the current date and time into the }
\CommentTok{# label 'org.opencontainers.image.created':}
...
\NormalTok{  org.opencontainers.image.created=$BUILD_DATE \textbackslash{}}
...
via environment variables {[}49{]}, or via wrappers using Docker, such
as Kliko {[}50{]}.
