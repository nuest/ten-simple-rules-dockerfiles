...
\begin{figure}[h]
\includegraphics[width=0.5\linewidth]{summary} \caption{Summary of the ten simple rules for writing Dockerfiles for reproducible data science.}\label{fig:summary}
\end{figure}
...
file (Dockerfile), which provides instructions for building an image.
Similar to using a compiled binary file to launch a program, the image
is then run to create a container instance. See Listing~\ref{lst:full}
for a full \texttt{Dockerfile}, which we will refer to throughout this
article.
