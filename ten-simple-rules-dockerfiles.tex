...
in science {[}17{]}. However, a Research Software Engineer's (RSE) skill
level is not the target audience for this work, but we hope to provide a
useful resource for more advanced container users as well. We encourage
you to reach out to your local or national RSE community if your
questions on sofware development in research go beyond the rules of this
work.
...
building images, comparable to \texttt{Makefile}s {[}26{]}. Container
...
have access to the \texttt{Dockerfile}, e.g., by using automated builds
on Docker Hub. We suggest to save a copy of the \texttt{Dockerfile}
within your project (see
...
that can help you get started. Avoid to \texttt{docker\ push} a locally
...
single line gets too long to comfortably read. You may choose to put
connecting syntactic elements at the beginning of lines to avoid faulty
placement or lingering \texttt{\&\&} as shown in Listing~2. Use long
...
performs one scoped well-documented action, e.g., download, compile, and
install \emph{one tool}. Each instruction will result in a new layer,
and reasonably grouped changes increase readability of the
...
include usage of bespoke scripts, \texttt{Makefile}s, or
...
re-build is also a good occassion to revisit the order of instructions,
...
browser (e.g., for Jupyter Hub {[}{\textbf{???}}{]}) are further
alternatives. This variety of approaches render seemingly more
convenient uncontainerised environments, i.e.~just using the local
machine, unnecessary in favor of reproducibility and portability.
...
While there are exceptions to the rule (cf.~{[}56{]}), it is generally a
...
slate for a new project. As an example, cookie cutter templates {[}57{]}
or community templates (e.g., {[}58{]}) can provide required structure
...
documentation of the computing environment can be very high {[}59{]}.
...
friendly ``preproducibility'' {[}64{]}. So please, make a best effort
...
thank Dav Clark who provided feedback on the preprint {[}65{]} of this
paper.
...
64. Stark PB. Before reproducibility must come preproducibility
...
65. Nüst D, Sochat V, Marwick B, Eglen S, Head T, Hirst T. Ten Simple
