...
\begin{flushleft}
{\Large
\textbf\newline{Ten Simple Rules for Writing Dockerfiles for
Reproducible Data
...
\\
Daniel Nüst\textsuperscript{\getAff{Institute for Geoinformatics,
University of Muenster, Muenster, Germany}}\textsuperscript{*},
Vanessa Sochat\textsuperscript{\getAff{Stanford Research Computing
Center, Stanford University, Stanford, CA, USA}},
Ben Marwick\textsuperscript{\getAff{Department of Anthropology,
University of Washington, Seattle, WA, USA}},
Stephen J. Eglen\textsuperscript{\getAff{Department of Applied
Mathematics and Theoretical Physics, University of Cambridge, Cambridge,
Cambridgeshire, GB}},
Tim Head\textsuperscript{\getAff{Wild Tree Tech, Zurich, CH}},
Tony Hirst\textsuperscript{\getAff{Department of Computing and
Communications, The Open University, GB}},
Benjamin D. Evans\textsuperscript{\getAff{School of Psychological
Science, University of Bristol, Bristol, GB}}\\
\bigskip
\bigskip
...
\sout{for building images}, comparable to a \texttt{Makefile}
{[}27{]}{\color{blue}, for building \emph{images} [1.1]}. Here, images
{\color{blue}are executable files that [1.1]} include the application,
e.g., the programming language interpreter needed to run a workflow, and
the system libraries required by an application to run. Thus, a
...
times. \sout{The images have a main executable exposed as an
``entrypoint'' that is started when they are run as stateful containers,
which are the running instances of Docker images.}
{\color{blue}Once an image is built or downloaded, it is then launched as a running instance known as a \emph{container}.
The images have a main executable exposed as an "entrypoint" that is started when they are run as stateful containers. [1.1]}
...
(see~\sout{Rule
6}{\color{blue}\hyperref[{rule:pinning}]{Rule~\ztitleref{rule:pinning}} [2.2]}
for ensuring the same software versions).
...
labels see~\sout{Rule
3}{\color{blue}\hyperref[{rule:document}]{Rule~\ztitleref{rule:document}} [2.2]}).
...
single line gets too long to comfortably read.
{\color{blue}Content spread across more and shorter lines also improves readability of changes in version control systems. [1.2]}
Further, use long versions of parameters for readability (e.g.,
\texttt{-\/-input} instead of \texttt{-i}). When you need to change a
directory, use \texttt{WORKDIR}, because it not only creates the
directory if it does not exist but also persists the change across
multiple \texttt{RUN} instructions.

Second, clarity \sout{is nearly always more important than
brevity}{\color{blue}of the steps within a Dockerfile is most important, and if it requires verbosity or adds to the final image size, that is an acceptable tradeoff [3.1]}.
For example, if your container uses a script to run a complex install
routine, instead of removing it from the container upon completion
\sout{,which is} {\color{blue}(a practice [3.1]} commonly seen in
production \texttt{Dockerfile}s aiming at small image sizes,
cf.~{[}12{]} {\color{blue}) [3.1]}, you should keep the script in the
...
commands\sout{,which installs} {\color{blue}to install [3.1]} software
and clean\sout{s} up afterwards. For example (a) the instruction updates
the database of available packages, installs a piece of software from a
package repository, and purges the cache of the package manager, or (b)
the instruction downloads a software's source archive, unpacks it,
builds and installs the software, and then removes the downloaded
archive and all temporary files. Although this pattern creates
instructions that may be hard to read, it is very common and can even
...
control system (see \sout{Rule
9}{\color{blue}\hyperref[{rule:publish}]{Rule~\ztitleref{rule:publish}} [2.2]}).
...
\begin{lstlisting}[language=docker,caption={Partial \texttt{Dockerfile} with examples for helpful comments. {\color{blue} [1.3]}},breaklines=true,label={lst:comments}]
...
...
\begin{lstlisting}[language=docker,caption={Partial \texttt{Dockerfile} with commonly used labels{\color{blue}; note the line breaks within the values (using the \texttt{\textbackslash} character), which were added to limit line length, are not preceded by a space character, as this space would appear in the value, whereas the line breaks between keys and values are separated by whitespace for readability [1.4]}.},breaklines=true,label={lst:labels}]
...
...
\hypertarget{section}{%
\subsection{\texorpdfstring{{\color{blue}Version Control [2.1]}}{}}\label{section}}

{\color{blue}Software can often be installed directly from a version controlled repository (e.g., GitHub, GitLab, or Mercurial). 
It's recommended to checkout a specific version, tag, or commit to ensure pinning a version for the repository.
For example, here is how to clone a specific release tag (`v3.6.1`) of the Singularity container software: [2.1]}
...
{\color{blue}In the case that you want to clone and checkout a specific commit, you can use the \texttt{checkout} command. [2.1]}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{RUN}\NormalTok{ pip install }\KeywordTok{\textbackslash{}}
  \VariableTok{geopy=}\NormalTok{=}\ExtensionTok{1.20.0} \KeywordTok{\textbackslash{}}
  \VariableTok{uszipcode=}\NormalTok{=}\ExtensionTok{0.2.2}
\end{Highlighting}
\end{Shaded}
...
the same repository (see \sout{Rule
9}{\color{blue}\hyperref[{rule:publish}]{Rule~\ztitleref{rule:publish}} [2.2]}).
You can also use package managers to install software from source code
...
\ExtensionTok{docker}\NormalTok{ run {-}{-}mount type=bind,src=}\VariableTok{$(}\BuiltInTok{pwd}\VariableTok{)}\NormalTok{/article{-}x{-}supplement/data,dst=/input{-}data,readonly }\KeywordTok{\textbackslash{}}
  \ExtensionTok{{-}{-}mount}\NormalTok{ type=bind,src=}\VariableTok{$(}\BuiltInTok{pwd}\VariableTok{)}\NormalTok{/outputs,dst=/output{-}data mycontainer}
\end{Highlighting}
\end{Shaded}
...
should be included in the example commands (see \sout{Rule
3}{\color{blue}\hyperref[{rule:document}]{Rule~\ztitleref{rule:document}} [2.2]}).
In these commands, you can also make sure to avoid issues with file
...
instructions for its usage (see \sout{Rule
3}{\color{blue}\hyperref[{rule:document}]{Rule~\ztitleref{rule:document}} [2.2]}).
To support \sout{advanced custom
configuration}{\color{blue}more complex configuration options []}, it is
helpful to expose settings via a configuration file, which can be bind
mounted from the host {[}51{]}, via environment variables (see
\sout{Rule
6}{\color{blue}\hyperref[{rule:document}]{Rule~\ztitleref{rule:document}} [2.2]}
and {[}53{]}), or via wrappers using Docker, such as Kliko {[}54{]}.
...
to date, e.g., updating versions of pinned dependencies (see \sout{Rule
6}{\color{blue}\hyperref[{rule:pinning}]{Rule~\ztitleref{rule:pinning}} [2.2]})
and regular image building (see above).
