...
performs 1 scoped action, e.g., download, compile, and install 1 tool.
This makes the lines of your \texttt{Dockerfile} a well-documented
...
\texttt{docker\ build} and \texttt{docker\ run} the image,
\textbf{within} the \texttt{Dockerfile}, either at the top or bottom
where the reader is likely to find them. Such documentation is
especially relevant if bind mounts, specific names, or ports are
important for using the container; see, for example, the final lines of
Listing~\ref{lst:full}. These instructions are not limited to
...
of usage). Usage instructions also provide a de facto way of testing
that your container works in a way that others can try out. The
...
versions of software is called \textbf{version pinning} (e.g., on
...
Installing from source is a useful way to install very specific
...
\href{https://docs.docker.com/engine/reference/builder/\#healthcheck}{\texttt{HEALTHCHECK}},
can further ensure proper functioning of your container.
...
sustainability of your work (cf.~{[}48{]}).

Most importantly, you should publish \textbf{all} files \texttt{COPY}ied
...
independent environment. CI increases stability and trust, and it allows
for images to be published automatically. Automation strategies exist to
build and test images for multiple platforms and software versions, even
with CI. Such approaches are often used when developing popular software
packages for a broad user base operating across a wide range of target
platforms and environments, and they can be leveraged if you expect your
workflow to fall into this category. Furthermore, the commit messages in
your version-controlled repository preserve a record of all changes to
the \texttt{Dockerfile}, and you can use the same versions in tags for
both the container's image and the git repository.
...
\href{https://docs.docker.com/storage/bind-mounts/}{\textbf{bind
mounts}}. Mounting these files is preferable to using the
...
Storing \textbf{data files} outside of the container allows handling of
...
A mount can also be used to access \textbf{output data} from a
...
mounts of directories or files, including options, as shown in the
following examples. If the target path exists within the image, the bind
mount will replace it for the started container. (Note, \texttt{\$HOME}
is an environment variable in UNIX systems representing the path to the
current user's home directory, e.g., \texttt{/home/moby}, and
\texttt{\$(pwd)} returns the current path.)
...
\hypertarget{rule-8-make-the-image-1-click-runnable}{%
\section*{Rule 8: Make the image 1-click
runnable}\label{rule-8-make-the-image-1-click-runnable}}
\addcontentsline{toc}{section}{Rule 8: Make the image 1-click runnable}
...
full workflows. For example, {[}49{]} demonstrates a container for
...
These 2 usages can be configured by the \texttt{Dockerfile}'s author and
exposed to the user based on the \texttt{Dockerfile}'s
...
command to print out usage. The container entrypoint should \textbf{not}
...
containers is that they can only provide 1 default entrypoint and
command. However, tools, e.g., The Scientific Filesystem {[}50{]}, have
...
a configuration file, which can be bind mounted from the host {[}49{]},
via environment variables (see
\hyperref[{rule:document}]{Rule~\ztitleref{rule:document}} and
{[}51{]}), or via wrappers using Docker, such as Kliko {[}52{]}.
...
Interactive graphical interfaces, such as
...
Interactive usage of a command-line interface is quite straightforward
to access from containers, if users are familiar with this style of user
interface. Running the container will provide a shell where a tool can
be used and where help or error messages can assist the user. For
example, complex workflows in any programming language can, with
suitable pre-configuration, be triggered by running a specific script
file. If your workflow can be executed via a command line client, you
may use that to validate correct functionality of an image in automated
builds, e.g., by using a small toy example and checking the output by
checking successful responses from HTTP endpoints provided by the
container, such as with an HTTP response code of \texttt{200}, or by
using a browser automation tool such as Selenium {[}53{]}.
...
natively supported by Singularity {[}54{]}, which can execute Docker
containers, or by leveraging supporting tools such as \texttt{x11docker}
{[}55{]}. Other alternatives include bridge containers {[}56{]} and
exposing a regular desktop via the browser (e.g., for Jupyter Hub
{[}57{]}). This variety of approaches renders seemingly more convenient
...
You can take advantage of \textbf{build caching} to avoid execution of
...
in order of least likely to change to most likely to change. Docker will
execute the instructions in the order that they appear in the
\texttt{Dockerfile}; when 1 instruction is completed, the result is
cached, and the build moves to the next one. If you change something in
the \texttt{Dockerfile} and rebuild the image, each instruction is
...
effectively by following a number of \textbf{good habits}, discussed in
this section. While there is no firm rule, if you use a container daily,
is good practice to rebuild that container every 1 or 2 weeks; this
helps identify breaking changes early and prevents multiple issues
...
you have properly documented the computing environment {[}58{]}. You
...
In case you need a setup or configuration for the first 2 habits, it is
...
command line {[}59{]} or as a web service {[}60{]}, are available and
...
snapshot of the actual environment you used at a specific point in time.
You should include instructions for how to import and run the workflow
based on the image archive and add your own image tags using semantic
versioning (see \hyperref[{rule:base}]{Rule~\ztitleref{rule:base}}) for
...
them likely to be preserved, but it is highly unlikely that over time
...
historians to emulate the Docker environment. Sharing the actual image
via a registry and a version-controlled \texttt{Dockerfile} together
allows you to freely experiment and continue developing your workflow
and keep the image up to date, e.g., updating versions of pinned
dependencies (see
...
To demonstrate the 10 rules, we maintain a collection of annotated
...
science research. Reproducibility in research is an endeavour of
...
``preproducibility'' {[}64{]}, which values openness, transparency and
...
freely break these rules if another approach makes more sense for their
use case. Also, we ask that researchers not overwhelm themselves by
trying to follow all the rules right away, but that they set up an
...
We thank Dav Clark who provided feedback on the preprint {[}65{]} and
Celeste~R. Brennecka from the Scientific Editing Service of the
University of Münster for her editorial support.
...
25. Project Jupyter, Bussonnier M, Forde J, Freeman J, Granger B, Head
T, et al. Binder 2.0 - Reproducible, interactive, sharable environments
for science at scale. Proceedings of the 17th Python in Science
Conference. 2018; 113--120.
...
49. Verstegen JA. JudithVerstegen/PLUC\_Mozambique: First release of
...
50. Sochat V. The Scientific Filesystem. GigaScience. 2018;7.
...
51. Knoth C, Nüst D. Reproducibility and Practical Adoption of GEOBIA
...
52. Molenaar G, Makhathini S, Girard JN, Smirnov O. Kliko---The
...
59. A rule-based linter for dockerfiles {[}Internet{]}. 2020. Available:
\url{https://github.com/projectatomic/dockerfile_lint}

\leavevmode\hypertarget{ref-hadolint}{}%
60. Dockerfile linter {[}Internet{]}. 2020. Available:
